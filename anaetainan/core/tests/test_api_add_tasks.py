...
def test_nao_deve_permitir_criar_task_sem_login(client):
    ...
    resp = client.post(
        "/api/core/tasks/add", {"new_task": "walk the dog"}
    )
...
def test_deve_criar_task_com_login(client, logged_jon):
    # Dado um usuario logado
    payload = {"description": "estudar pytest"}

    # Quando adicionamos um item
    resp = client.post(
        "/api/core/tasks/add", payload, content_type="application/json"
    )
...
def test_deve_falhar_com_input_invalido(client, logged_jon):
    ...
    assert msg == {
        "message": "[INVALID INPUT] body.task.description: field required (value_error.missing)",
    }
...
def test_deve_falhar_com_input_menor_que_minimo_necessario(client, logged_jon):
    ...
    assert resp.status_code == 422  # BAD REQUEST
    assert resp.json() == {
        "message": "[INVALID INPUT] body.task.description: It must be at least 3 characteres long. (value_error)",
    }
...
def test_deve_falhar_quando_description_contem_algo_diferente_de_string(
    ...
    assert resp.json() == {
        "message": "[INVALID INPUT] body.task.description: str type expected (type_error.str)",
    }
...
def test_deve_receber_erro_enviado_pela_classe_de_servico(client, logged_jon):
    ...
    # Quando tentamos adicionar
    with mock.patch(
        "anaetainan.core.service.tasks_svc.add_task"
    ) as add_task_mock:
